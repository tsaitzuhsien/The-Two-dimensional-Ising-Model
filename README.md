...
$$H(s) = -\sum_{i \in L} \sum_{j \in n(i)} J_{i, j} f(s_i, s_j) - \mu \sum_{i \in L} h_i s_i$$

| The term | Its meaning |
| :---: | --- |
| $L$ | A set consisting of all atoms in the lattice |
| $s$ | A set consisting of the spin direction $s_i$ of each atom $i$ |
...
$$H(s) = -J\sum_{i \in L} \sum_{j \in n(i)} s_i s_j$$
...
4. If there is an atom in $C$ that has not been assigned as $s$, assign it as $s$ and go back to step 4.
